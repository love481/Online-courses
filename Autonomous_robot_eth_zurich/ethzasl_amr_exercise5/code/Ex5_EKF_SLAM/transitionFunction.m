...
f=zeros(size(x,1),1);
f(:) =[x(1)+((u(1)+u(2))*cos(x(3)+((u(2)-u(1))/(2*b))))/2;...
       x(2)+((u(1)+u(2))*sin(x(3)+((u(2)-u(1))/(2*b))))/2;...
        x(3)+(u(2)-u(1))/b;...
        x(4:end,1)];

F_x = eye(size(x,1));
F_x(1:3,1:3) = [1 0 -((u(1)+u(2))*sin(x(3)+((u(2)-u(1))/(2*b))))/2;...
       0 1 ((u(1)+u(2))*cos(x(3)+((u(2)-u(1))/(2*b))))/2;...
       0 0 1];

F_u = zeros(size(x,1),2);
F_u(1:3,1:2) = [((cos(x(3)+((u(2)-u(1))/(2*b))))/2)+(((u(1)+u(2))/(4*b))*sin(x(3)+((u(2)-u(1))/(2*b)))) ...
